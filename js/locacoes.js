document.addEventListener('DOMContentLoaded', () => {
  ...
  const logoutBtn = document.getElementById('logout-button'); // ✅ Adicionado seletor do botão sair
  ...
  function initializeListeners() {
    ...
    inputPesquisa.addEventListener('keyup', () => {
      ...
      document.querySelectorAll('.turma-card').forEach(card => {
        ...
        // Nota: Certifique-se de ter a classe .hidden-by-search { display: none !important; } no seu CSS
        // Ou use card.style.display = 'none' / 'block'
        if (nome.includes(termo)) {
          card.style.display = 'block'; // Fallback se a classe não existir
          card.classList.remove('hidden-by-search');
        } else {
          card.style.display = 'none'; // Fallback
      ...
      logoutBtn.addEventListener('click', (e) => {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            try {
              // Tenta notificar a API (opcional, mas boa prática)
              if (TOKEN) {
